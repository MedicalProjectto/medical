...
class WeiXin {
  ...
  private function getAccessToken() {
    ...
    $cons = new CConstant('/tmp');
    $consTrr = $cons->getConstant('weixin');

    $id = -1;
    if (!empty($consTrr)) {    
      foreach ($consTrr as $key => $value) {
        if ($value['appid'] == $this->appid) {
          if ($value['valid_time'] > time()) {  
            $this->access_token = $value['access_token'];
            $this->valid = $value['valid_time'];
            return true;
          }
          $id = $value['id'];
          break;
      ...
      if ($this->appid && $this->appsecret) {
        ...
        if (isset($access->access_token) && isset($access->expires_in)) {
          ...
          $data = array('appid'=>$this->appid, 'access_token'=>$access->access_token, 'parentid'=>0, 'valid_time'=>$this->valid);
        } else {
          return false;
        }
    } else {
    ...
    if (empty($consTrr) || $id == -1) {
      $cons->appendConstant('weixin', $data);
    } else {
      $cons->updateConstant('weixin', $id, $data);
    }
   ...
   * @return bool|mixed
   */
  public function sendTempMessage($openid, $template_id, $url, $data, $topcolor = '#00BFFF')
  {
    if ($this->getAccessToken())
    {
      ...
      return self::post($url, urldecode($json_template));
    }
